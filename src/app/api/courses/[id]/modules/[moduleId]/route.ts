...
export async function GET(
  ...
  try {
    ...
    return errorResponse(status === 401 ? 'UNAUTHORIZED' : 'INTERNAL_ERROR', message, status);
  }
}
...
export async function POST(
  ...
  try {
    ...
    const status = message.includes('Missing') || message.includes('expired') || message.includes('Invalid') ? 401 : message.includes('Forbidden') ? 403 : 500;
    return errorResponse(status === 401 ? 'UNAUTHORIZED' : status === 403 ? 'FORBIDDEN' : 'INTERNAL_ERROR', message, status);
  }
}
